...
- LoL未起動/未ログイン: `League of Legendsを起動してください`。起動後も切り替わらない場合はSettingsでゲームのインストールディレクトリ設定を確認するよう案内する
- LoLログイン後かつドラフト開始前: `こんにちは <ユーザー名>` と、ChampionPoolの登録、対応ドラフトの開始、チャンピオン選択開始後にBAN・Pick候補と推奨ビルドを確認する流れを表示。画面下部には、Championsでビルド・ルーン・スキル優先度・対面データを確認できること、Statsで自分とレーン対面の戦績を確認できることも案内する
...
- バンピック中の StatsAPI おすすめ詳細レスポンスは renderer 内で条件別にキャッシュし、同一の `position + championId + opponentChampionId` 条件に戻ったときは再リクエストせずキャッシュ済みレスポンスを表示する
