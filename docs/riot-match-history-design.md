...
`season` は `Download recent match` 右側のプルダウンから手動実行する。match id 一覧を `count=100` でページングし、現在年の `1/1 00:00 JST` を `startTime` として使う。
...
6. recent は 5並列 / 350ms batch delay、season は 5並列 / batch delay なしで detail を取得する
7. 429 では `Retry-After` を優先して待ち、待機開始時に取得済み分を正規化する
8. raw cache を更新する
9. 正規化して `match-history/{localPuuid}.json` に保存する
10. champion ごとの集計関数を作る
11. ChampionPool 表示に戦績を追加する
12. Draft の ChampSelect に非AI候補ランキングを追加する
