...
ゲーム中の対面おすすめビルドで対面判定を追う場合は、`lcu-logic.ts` の共通 resolver を起点に確認します。AI対面分析の 2v2 判定と、StatsAPI 向けの同ロール対面判定は同じ gameflow / champ-select 補完経路を使い、用途ごとに最終的な相手チャンピオンの選び方だけを分けています。
...
- `ui/in-game-view.ts` はゲーム中画面の StatsAPI おすすめ表示を担当します。detail API 取得時は `championId / position / opponentChampionId` に加えて、local player のキーストーンを `keystoneId` として付け、通常おすすめ・対面おすすめの両方を実際に使うキーストーンで絞り込みます。キーストーン取得元は `/lol-perks/v1/currentpage` を優先し、取れない場合だけ ChampSelect member の rune/perk 情報へフォールバックします。ゲーム中のチャンピオンカード付近には、選択中キーストーンをアイコン付きで表示します。デバッグ時は renderer 側で `In-game recommendation context resolved` / `In-game recommendation request started`、main 側で `StatsAPI request started/completed` を見ると、どの `keystoneId` と `opponentChampionId` で問い合わせたか追えます。
...
- `docs/in-game-view-mock.html`: 試合を開始しなくてもゲーム中画面の UI をブラウザで確認するための静的モック。左側のチャンピオン情報とスキル欄は縦方向を圧縮した高密度レイアウトで、右側のアイテム欄はタブ内メトリクス付きの 2 段 3 列レイアウトとテーマ別配色を確認できる
