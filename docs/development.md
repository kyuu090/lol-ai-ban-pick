...
- `ui/in-game-view.ts` はゲーム中画面の StatsAPI おすすめ表示を担当します。detail API 取得時は `championId / position / opponentChampionId` に加えて、local player の ChampSelect rune/perk 情報から keystone が取れている場合は `keystoneId` も付け、実際に使うキーストーンでビルドとスキル候補を絞り込みます。
