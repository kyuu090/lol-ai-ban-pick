...
最初から完全な統計基盤を作るのではなく、LCU match history から取得できる自分の直近戦績と、必要に応じた静的 JSON/CSV から始める。将来的に Riot API や外部集計データへ差し替えられる構造を目指す。
...
- Raw data: LCU match history や将来の Riot API などから取得した試合そのもの
...
MVP では LCU match history から正規化済み自己戦績を作り、外部メタ統計は省略または静的ファイルで持ってもよい。
...
LCU match history から作る、ユーザー本人の champion 別実績。
...
初期実装では `app.getPath('userData')/match-history.json` に正規化済み match records を保存し、起動時または取得完了時に集計する。
...
2. LCU match history から自分の直近試合を取得する
3. match history を正規化して `match-history.json` に保存する
4. champion ごとの自己戦績 `user_champion_stats` を集計する
5. ChampionPoolに自己戦績を表示する
6. 現在の champ select 状態から、味方 pick、敵 pick、ban 済み champion を抽出する
7. 現在ロールのChampionPoolから候補 champion を抽出する
8. 候補 champion に対して `personal_performance_score` を計算する
9. UI に推薦理由として、自己戦績、直近成績、サンプル数、ChampionPool内候補であることを表示する
10. 後から静的 `meta_stats.json` や外部統計を追加し、`champion_strength_score` と `counter_score` を足す

## 注意点

### BAN 率の取得は難しい

LCU match history だけでは、ranked solo queue 全体の ban rate を集計できない。通常の match-v5 データでも、全体 ban rate を簡単に集計するには多くの試合データが必要になる。
...
LCU match history や Riot API の lane/role 情報は常に完璧ではない。

特に off-meta pick や lane swap ではズレる可能性がある。

初期実装では LCU の判定を信じ、後から補正ロジックを追加する。
...
- Riot API Match-V5 fallback を追加する
