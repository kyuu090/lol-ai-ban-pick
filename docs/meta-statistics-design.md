...
2. `meta_stats.json` を静的ファイルとして用意する
3. `championStats` と `matchupStats` を読み込める provider を作る
4. 現在の champ select 状態から、味方 pick、敵 pick、ban 済み champion を抽出する
5. 現在ロールのChampionPoolから候補 champion を抽出する
6. 候補 champion に対して `champion_strength_score` と `counter_score` を計算する
7. UI に推薦理由として、勝率差分、相性差分、サンプル数、ChampionPool内候補であることを表示する
8. 後から SQLite 集計バッチを追加し、静的 JSON を生成する
