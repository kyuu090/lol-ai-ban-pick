...
StatsAPIによるチャンピオン統計は、メインナビゲーションの「チャンピオン」画面から参照する。チャンピオンを選択した後、「ビルド」「マッチアップ分析」「タイムライン分析」を切り替え、対面一覧から特定対面の時間推移へ掘り下げられる。ドラフト画面からの参照は想定しない。

画面遷移、使用API、表示指標の詳細は [チャンピオン分析画面](./champion-analysis-view.md) を参照する。
...
大量の matchup sample がない段階でも、AIはドラフト理解の補助として使える。ただし、AIに勝率や matchup delta を作らせない。

AIに任せる範囲:

- バンピック状況の整理
- 味方/敵構成の強み、不足、脅威の説明
- ChampionPool候補の長所短所整理
- サンプル不足の注意喚起
- 試合開始時の大まかなゲーム展開予測
...
- matchup delta の創作
- サンプル不足の断定的なカウンター評価
- ChampionPoolや候補外championの過度な推薦

AI補助は、構成の噛み合い、ダメージバランス、CC、フロントライン、スケーリングなど、統計だけでは表しにくい要素に限定する。
