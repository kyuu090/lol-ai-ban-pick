...
- GameStart / InProgress 開始時の teamOne / teamTwo と selectedPosition から、ゲーム側が持つ同一ポジション対面を判定する

ノーマルドラフトの実ログでは、`phase: "GameStart"` の `/lol-gameflow/v1/session` に `gameData.teamOne` / `gameData.teamTwo` が入り、各 participant に `championId` と `selectedPosition` が含まれていた。
...
gameData: {
  teamOne: [
    ...
    { championId: 50, selectedPosition: "TOP" },
    { championId: 21, selectedPosition: "BOTTOM" },
    { championId: 56, selectedPosition: "JUNGLE" },
    { championId: 89, selectedPosition: "UTILITY" }
  ],
  teamTwo: [
    { championId: 36, selectedPosition: "JUNGLE" },
    { championId: 136, selectedPosition: "TOP" },
    { championId: 800, selectedPosition: "BOTTOM" },
    { championId: 268, selectedPosition: "MIDDLE" },
    { championId: 26, selectedPosition: "UTILITY" }
  ],
  playerChampionSelections: [
    { championId: 268, puuid: "..." }
  ]
}
```

この例では local summoner の `puuid` が `teamTwo` の `championId: 268` / `selectedPosition: "MIDDLE"` と一致するため、相手チームは `teamOne`。`teamOne` の同じ `selectedPosition: "MIDDLE"` は `championId: 238` なので、ゲーム側情報から見た対面は Zed と判定できる。

注意:

- ChampSelect 中の `tbdGameDtoV1` では、味方側 `assignedPosition` は取れたが、敵側は championId のみで position は取れなかった。
- 対面判定は `GameStart` 以降の `/lol-gameflow/v1/session` を優先する。
- `teamOne` / `teamTwo` のどちらが味方かは、local summoner の `puuid` が含まれる側で判定する。
- `selectedPosition` が空、または同一 position の敵が見つからない場合は、AI対面分析を発火しない。
...
`GameStart` または `InProgress` の `/lol-gameflow/v1/session` で、自分と敵チームの同一 `selectedPosition` champion が判定できたタイミングで、BFF に対面分析を依頼する。

発火条件:

- `gameData.teamOne` と `gameData.teamTwo` がどちらも配列
- local summoner の `puuid` がどちらかの team に存在する
- local participant に `championId > 0` と `selectedPosition` がある
- 相手 team に同じ `selectedPosition` かつ `championId > 0` の participant がいる
- 同じ gameId / local champion / opponent champion / selectedPosition では二重送信しない
...
```js
{
  phase: "lane_matchup",
  localPlayer: {
    champion: { championId: 268, championName: "Azir" },
    position: "MIDDLE"
  },
  opponent: {
    champion: { championId: 238, championName: "Zed" },
    position: "MIDDLE"
  },
  gameId: 589378305
}
...
これはユーザーが ChampSelect 中に手動指定する lane opponent とは別扱いにする。ゲーム側の `selectedPosition` が揃った時点での自動対面分析として扱う。
