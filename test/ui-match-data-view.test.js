...
function createElements() {
  return {
    ...
    matchDataRange: { classList: createClassList(), hidden: false, textContent: '' },
    matchDataProgress: { dataset: {}, hidden: false, textContent: '', title: '' },
...
test('match data view updates status controls and menu state', () => {
  ...
  assert.equal(elements.matchDataProgress.title, 'loading');
  assert.equal(elements.matchDataProgress.hidden, false);
  assert.equal(elements.matchDataRange.hidden, true);
  ...
  assert.equal(elements.matchDataRange.hidden, false);
